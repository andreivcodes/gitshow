...
import { COLOR_MAP, type ContributionData, type ContributionValue } from "./contributions_types";
...
export async function contribData(username: string): Promise<ContributionData> {
  ...
  const data: ContributionData = {
    ...
    contributions: (() => {
      const parseDay = (day: Element): { date: number[]; value: ContributionValue } => {
        const $day = $(day);
        const date =
          $day
            .attr("data-date")
        ...
        const value = {
          ...
          count: parseInt($day.text().split(" ")[0] as string, 10) || 0,
          color,
